...
std::string SeMIRTWorkerRequest::EncodeTo() const {
  json::JSON ret;
...
std::string SeMIRTWorkerResponse::EncodeTo() const {
  json::JSON ret;
  ret["status"] = status;
  ret["output"] = output;
  if (!aux.empty()) {
...
std::string SeMIRTFnPackerRequest::EncodeTo() const {
  json::JSON req;
...
std::string SeMIRTFnPackerResponse::EncodeTo() const {
  json::JSON ret;
  if (!status) {
    ret["error"] = output;
    return ret.dump();
