...
thread_local semirt_worker::ModelRT* model_rt;
// --- execution context --- //
...
sesemi::SimpleCache<semirt_worker::ModelContext> model_cache{1};
// caching the last pair of model-request key pair in use.
sesemi::SimpleCache<KeySet> key_cache{1};
// shared ra channel
...
// only allow single permitted model under equal_exec_time setting
  ...
  // if (input_data_size != sesemi::get_aes_encrypted_size(INPUT_BUF_SZ)) {
  //   size_t sz = sesemi::get_aes_encrypted_size(INPUT_BUF_SZ);
  ...
  if (cache_ret == 0) {
    ...
    auto create_key_request_msg = [&]() -> std::string {
      return sesemi::GetKeyRequest(std::string(user_id, user_id_size),
  ...
  unsigned char* decrypt_input = NULL;
  {
    ...
    auto ret = sesemi::decrypt_content_with_key_aes(
  ...
  int ret = 0;
  semirt_worker::ModelContext* decrypt_model = nullptr;
#if EQUAL_EXEC_TIME
  if (!initialized_once) {
  ...
  if (cache_ret == 0) {
    ...
    auto model_context = semirt_worker::load_model(
  ...
  ret = (sgx_status_t)execute_inference(
      (char*)decrypt_input, sesemi::get_aes_decrypted_size(input_data_size),
  ...
  *output_size =
      (ret == SGX_SUCCESS) ? sesemi::get_aes_encrypted_size(OUTPUT_BUF_SZ) : 0;
...
sgx_status_t enc_get_encrypted_prediction(uint8_t* prediction, size_t size) {
  size_t whole_cipher_text_size = sesemi::get_aes_encrypted_size(OUTPUT_BUF_SZ);
  ...
  sgx_status_t ret = sesemi::encrypt_content_with_key_aes(
