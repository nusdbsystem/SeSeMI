...
int main(int argc, char* argv[]) {
  ...
  if (argc == 3) {
    ...
    if (sesemi::ReadFileToString(sample_request_path, &sample_request) == -1) {
  ...
  semirt_worker::WorkerU worker(
      WORKER_ENCLAVE_FILENAME,
      std::unique_ptr<sesemi::Store>(sesemi::OpenFsStore(
  ...
  for (uint64_t i = 0; i < INFERENCE_COUNT; i++) {
    handlers.emplace_back(&semirt_worker::WorkerU::Handle, &worker, i,
...
// sudo ./app tmp/ ../example/tvm-mb/inference_req.json
