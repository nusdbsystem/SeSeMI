...
#include "worker/inference-runtime/examples/tvm-bert/src/common/abstract.h"
#include "worker/inference-runtime/examples/tvm-bert/src/common/bert_input.h"
#include "worker/inference-runtime/model/model.h"
#include "worker/inference-runtime/tvm_crt/src/common/abstract.h"
#include "worker/inference-runtime/tvm_crt/src/common/bundle.h"
...
int execute_inference(const char* input_src, size_t input_src_size,
  ...
  // BERT input
  BertInput input_viewer;
  view_bert_input(&input_viewer, (float*)input_src, input_src_size / 4);

  DLTensor words;
  words.data = input_viewer.words;
  words.device = dev;
  words.ndim = 2;
  words.dtype = dtype;
  int64_t words_shape[2] = {1, input_viewer.max_seq_length};
  words.shape = words_shape;
  words.strides = NULL;
  words.byte_offset = 0;

  DLTensor segments;
  segments.data = input_viewer.segments;
  segments.device = dev;
  segments.ndim = 2;
  segments.dtype = dtype;
  int64_t segments_shape[2] = {1, input_viewer.max_seq_length};
  segments.shape = segments_shape;
  segments.strides = NULL;
  segments.byte_offset = 0;

  DLTensor valid_len;
  // valid_len.data = input_valid_len;
  valid_len.data = input_viewer.valid_len;
  valid_len.device = dev;
  valid_len.ndim = 1;
  valid_len.dtype = dtype;
  int64_t valid_len_shape[1] = {1};
  valid_len.shape = valid_len_shape;
  valid_len.strides = NULL;
  valid_len.byte_offset = 0;

  tvm_runtime_set_input(handle, "data0", &words);
  tvm_runtime_set_input(handle, "data1", &segments);
  tvm_runtime_set_input(handle, "data2", &valid_len);
  ...
  tvm_runtime_get_output(handle, 1, &output);
