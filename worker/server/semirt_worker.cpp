...
SeMIRTWorker::~SeMIRTWorker() { Close(); }

bool SeMIRTWorker::Initialize() {
  initialized_ = worker_->Initialize();
  return true;
}
bool SeMIRTWorker::Handle(const std::string& url, const std::string& input,
                         std::string* output) {
  bool error = false;
  std::string res;
...
void SeMIRTWorker::Close() { worker_->Close(); }
