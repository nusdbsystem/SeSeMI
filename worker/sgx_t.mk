...
COMMON_INCLUDE_FLAGS = -Isrc/trusted -I${PROJECT_ROOT_DIR}/include -I$(PROJECT_ROOT_DIR)/include/worker/trusted -I${SESEMI_ROOT} -I$(SGX_RA_TLS_DIR)/include
...
src/trusted/Enclave_t.c: $(SGX_EDGER8R) ${PROJECT_ROOT_DIR}/include/worker/trusted/Enclave.edl
	@echo Entering ./src/trusted
	cd ./src/trusted && $(SGX_EDGER8R) --trusted ${PROJECT_ROOT_DIR}/include/worker/trusted/Enclave.edl --search-path $(PROJECT_ROOT_DIR)/include --search-path $(SGX_SDK)/include --search-path $(SGXSSL_DIR)/include --search-path $(SGX_RA_TLS_DIR)/include --search-path $(SESEMI_ROOT) --search-path $(INFERENCE_RT_INCLUDE_DIR) $(Enclave_Search_Dirs)
	@echo "GEN  =>  $@"
...
src/trusted/base64.o: ${SESEMI_ROOT}/utils/base64.cpp
	$(CXX) ${Enclave_Cpp_Flags} -c $< -o $@
	@echo "CXX <= $<"

src/trusted/json.o: ${SESEMI_ROOT}/utils/json.cpp
	$(CXX) ${Enclave_Cpp_Flags} -c $< -o $@
	@echo "CXX <= $<"

src/trusted/tcrypto_ext.o: ${SESEMI_ROOT}/utils/tcrypto_ext.cpp
	$(CXX) ${Enclave_Cpp_Flags} -c $< -o $@
	@echo "CXX <= $<"

src/trusted/channel_client.o: $(SESEMI_ROOT)/ratls-channel/common/channel_client.cpp
	$(CXX) $(Enclave_Cpp_Flags) $() -c $< -o $@
	@echo "CXX  <=  $<"

src/trusted/keyservice_worker.o: ${SESEMI_ROOT}/message/keyservice_worker.cpp
	$(CXX) ${Enclave_Cpp_Flags} -c $< -o $@
	@echo "CXX <= $<"
## build files needed from other directory
