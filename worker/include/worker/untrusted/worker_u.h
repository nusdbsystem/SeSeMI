...
#ifndef SESEMIWORKER_UNTRUSTED_WORKER_H_
#define SESEMIWORKER_UNTRUSTED_WORKER_H_
 ...
 public:
  WorkerU(const std::string& enclave_file_name,
      ...
      : initialize_(false),
        ...
        eid_(0),
        model_store_(sesemi::OpenFsStore(std::move(remote_store_path),
                                 std::move(local_store_path))) {}

  WorkerU(const std::string& enclave_file_name,
          std::unique_ptr<sesemi::Store> model_store)
      : initialize_(false),
        enclave_file_name_(std::move(enclave_file_name)),
        eid_(0),
...
#endif  // SESEMIWORKER_UNTRUSTED_WORKER_H_
