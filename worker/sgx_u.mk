...
### Project Settings ###
COMMON_INCLUDE_FLAGS = -I. -I${PROJECT_ROOT_DIR}/include -I${SESEMI_ROOT} -I$(SESEMI_ROOT)/ratls-channel/common -I./src/untrusted
...
src/untrusted/Enclave_u.c: $(SGX_EDGER8R) ${PROJECT_ROOT_DIR}/include/worker/trusted/Enclave.edl
	@echo Entering ./untrusted
	cd ./src/untrusted && $(SGX_EDGER8R) --untrusted ${PROJECT_ROOT_DIR}/include/worker/trusted/Enclave.edl --search-path ${PROJECT_ROOT_DIR}/include/ --search-path $(SGX_SDK)/include --search-path $(SGXSSL_DIR)/include --search-path $(SGX_RA_TLS_DIR)/include --search-path ${SESEMI_ROOT} --search-path $(INFERENCE_RT_INCLUDE_DIR) $(Enclave_Search_Dirs)
	@echo "GEN  =>  $@"
...
src/untrusted/base64.o: ${SESEMI_ROOT}/utils/base64.cpp
	$(CXX) $(App_Cpp_Flags) -c $< -o $@
	@echo "CXX <= $<"

src/untrusted/fileutil.o: ${SESEMI_ROOT}/utils/fileutil.cpp
	$(CXX) $(App_Cpp_Flags) -c $< -o $@
	@echo "CXX <= $<"

src/untrusted/hexutil.o: ${SESEMI_ROOT}/utils/hexutil.cpp
	$(CXX) $(App_Cpp_Flags) -c $< -o $@
	@echo "CXX <= $<"

src/untrusted/json.o: ${SESEMI_ROOT}/utils/json.cpp
	$(CXX) $(App_Cpp_Flags) -c $< -o $@
	@echo "CXX <= $<"

src/untrusted/channel_client_ocalls.o: ${SESEMI_ROOT}/ratls-channel/untrusted/channel_client_ocalls.cpp
	$(CXX) $(App_Cpp_Flags) -c $< -o $@
	@echo "CXX <= $<"

src/untrusted/semirt_worker.o: ${SESEMI_ROOT}/message/semirt_worker.cpp
	$(CXX) $(App_Cpp_Flags) -c $< -o $@
	@echo "CXX <= $<"

src/untrusted/fs_store.o: ${SESEMI_ROOT}/store-client/fs_store.cpp
	$(CXX) $(App_Cpp_Flags) -c $< -o $@
	@echo "CXX <= $<"
...
App_Cpp_Objects := src/untrusted/base64.o src/untrusted/fileutil.o src/untrusted/hexutil.o src/untrusted/json.o src/untrusted/fs_store.o src/untrusted/semirt_worker.o src/untrusted/worker_u.o src/untrusted/ocalls.o src/untrusted/channel_client_ocalls.o
...
server/sgx/service.o: $(SESEMI_ROOT)/server/service.cpp
	$(CXX) $(App_Cpp_Flags) $(Server_Additional_Include_Flags) -c $< -o $@
	@echo "CXX <= $<"

server/sgx/server.o: $(SESEMI_ROOT)/server/server.cpp
	$(CXX) $(App_Cpp_Flags) $(Server_Additional_Include_Flags) -c $< -o $@
	@echo "CXX <= $<"

server/sgx/semirt_worker.o: server/semirt_worker.cpp
	$(CXX) $(App_Cpp_Flags) $(Server_Additional_Include_Flags) -c $< -o $@
	@echo "CXX <= $<"
...
Server_Objects := server/sgx/service.o server/sgx/server.o server/sgx/semirt_worker.o server/sgx/main.o

semirt_server: $(Server_Objects) libuntrusted_worker.a
...
### Clean command ###
clean:
	rm -f app src/untrusted/*.o $(App_Cpp_Objects) src/untrusted/Enclave_u.* libuntrusted_worker.a server/sgx/*.o semirt_server
