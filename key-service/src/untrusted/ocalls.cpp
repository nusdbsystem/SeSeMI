...
void ocall_save_kv(const void* key_buf, size_t key_len, const void* value_buf,
  ...
  auto key_b64 = sesemi::base64_encode((const uint8_t*)key_buf, key_len);
  auto value_b64 = sesemi::base64_encode((const uint8_t*)value_buf, value_len);
  printf("Save key value: key: %s, value: %s\n", key_b64.c_str(),
         value_b64.c_str());
  auto store = static_cast<sesemi::Store*>(store_ptr);
  store->Put(
      sesemi::CreateSealedFileName(std::string((const char*)key_buf, key_len)),
...
void ocall_get_kv(const void* key_buf, size_t key_len, void* value_buf,
  ...
  std::string value;
  auto store = static_cast<sesemi::Store*>(store_ptr);
  if (store->Get(sesemi::CreateSealedFileName(
