...
int main(int argc, char* argv[]) {
  ...
  sesemi::Store* store = sesemi::OpenFsStore(store_path);

  std::thread client_service =
      std::thread(sesemi::ServeClient, id, store, CON_CLIENT_CONNECT);
  std::thread worker_service =
      std::thread(sesemi::ServeWorker, id, store, CON_WORKER_CONNECT);
