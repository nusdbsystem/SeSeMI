...
bool load_and_unseal_data(const std::string& key, std::string* output,
  ...
  uint32_t unseal_buf_size =
      sesemi::get_unsealed_data_size((const uint8_t*)value_buf);
...
sgx_status_t seal_and_save_data(const std::string& key,
  ...
  int buf_size = sesemi::get_sealed_data_size(
  ...
  auto sgxStatus = sesemi::seal_data(
...
RequestExecCtx handle_user_register(const std::string& id_key, void* store) {
  // calculate the hash
  std::string user_id;
  auto id_key_bytes = sesemi::base64_decode(
...
RequestExecCtx decrypt_payload(const std::string& user_id,
  ...
  // decrypt the buffer
  uint8_t* plain_txt;
  auto decode_bytes = sesemi::base64_decode(
      reinterpret_cast<const uint8_t*>(payload.c_str()), payload.size());
  status = sesemi::decrypt_content_with_key_aes(
  ...
  output->assign(reinterpret_cast<const char*>(plain_txt),
                 sesemi::get_aes_decrypted_size(decode_bytes.size()));
  free(plain_txt);
  plain_txt = NULL;
...
int send_client_reply(WOLFSSL* ssl, const RequestExecCtx& ctx) {
  auto reply =
      (ctx.sgxStatus != SGX_SUCCESS)
          ? sesemi::KeyServiceReply(false,
                                   "Request failed: SGX error: " + ctx.msg)
                .EncodeTo()
          : ((ctx.request_failure)
                 ? sesemi::KeyServiceReply(false, "Request failed: " + ctx.msg)
                       .EncodeTo()
                 : sesemi::KeyServiceReply(true, "Request success").EncodeTo());
...
sgx_status_t enc_client_service(WOLFSSL* ssl, void* store) {
  ...
  sesemi::KeyServiceRequest req = sesemi::DecodeKeyServiceRequest(
      std::string(user_request_buf, strlen(user_request_buf)));
  if (req.type_ == sesemi::USER_REGISTER) {
  ...
  switch (req.type_) {
    ...
    case sesemi::ADD_REQUEST_KEY: {
      sesemi::AddRequestKeyRequest payload =
          sesemi::DecodeAddRequestKeyRequest(std::move(plain_payload));
    ...
    case sesemi::UPSERT_MODEL_KEY: {
      sesemi::UpsertModelKeyRequest payload =
          sesemi::DecodeUpsertModelKeyRequest(std::move(plain_payload));
    ...
    case sesemi::GRANT_MODEL_ACCESS: {
      sesemi::GrantModelAccessRequest payload =
          sesemi::DecodeGrantModelAccessRequest(std::move(plain_payload));
      if (!check_model_id(payload.model_id_, req.user_id_)) {
        ctx.request_failure = true;
        ctx.msg = "model id not binded to the owner id";
        break;
...
inline int send_worker_failure_reply(WOLFSSL* ssl,
  ...
  auto reply = sesemi::KeyServiceWorkerReply(std::move(error_msg)).EncodeTo();
  assert(reply.size() < INT_MAX);
  wolfSSL_write(ssl, reply.data(), static_cast<int>(reply.size()));
  return 0;
}
...
sgx_status_t enc_worker_service(WOLFSSL* ssl, void* store) {
  ...
  sesemi::GetKeyRequest req = sesemi::GetKeyRequest::DecodeFrom(
  ...
  // send reply
  auto reply = sesemi::KeyServiceWorkerReply(req.user_id(), user_decrypt_key,
