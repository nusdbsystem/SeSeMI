...
std::string build_response(sesemi::response_code_t code,
  ...
  switch (code) {
    ...
    case sesemi::FORBIDDEN:
      return build("403 Forbidden", std::move(msg));
    case sesemi::NOT_FOUND:
...
bool Server::Init() {
  ...
  return service_->Init() == sesemi::OK;
}

void Server::OnWork(uv_work_t* req) {
  client_t* client = (client_t*)req->data;
  sesemi::response_code_t code;
  std::string msg;
#ifndef NDEBUG
