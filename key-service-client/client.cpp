...
int main(int argc, char* argv[]) {
  ...
  if (sesemi::ReadFileToString(request_json_path, &request_str) == -1) {
  ...
  if (sesemi::ReadFileToString(config_json_path, &config_json_str) == -1) {
  ...
  if (status == 0) {
    auto reply = sesemi::DecodeKeyServiceReply(reply_msg);
